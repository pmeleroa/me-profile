...
validado por mí.". El texto resultante MUST ser:
...
- **WHEN** un visitante abre cualquier página pública que incluye el footer (portada, 404, contacto, secciones, listado del blog, artículo del blog o página de skill)
...
- **THEN** "Pablo Melero Alonso" enlaza a la portada del sitio y "Astro" enlaza a `https://astro.build` y se abre en una pestaña nueva con `rel="noopener"`

#### Scenario: Contenido esencial sin JavaScript
- **WHEN** un visitante carga una página con JavaScript desactivado
- **THEN** el texto completo del footer está presente en el HTML servido
